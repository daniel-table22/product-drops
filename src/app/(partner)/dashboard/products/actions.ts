...
import { CATEGORY_PHOTO_FILES } from "@/lib/category-photos";
...
// Photos are pulled from the shared CATEGORY_PHOTO_FILES map — keeps
// onboarding previews and item autofill in lockstep.
...
export async function seedItems(
  ...
  const photos = CATEGORY_PHOTO_FILES[category];
  if (!items || !photos) return { inserted: 0, error: `Unknown category: ${category}` };
  ...
  const n = Math.min(items.length, photos.length);
  const rows = items.slice(0, n).map((it, idx) => ({
    partner_id: partner.id,
    name: it.name,
    description: it.description,
    photo_url: `/preview-photos/${category}/${encodeURIComponent(photos[idx])}`,
